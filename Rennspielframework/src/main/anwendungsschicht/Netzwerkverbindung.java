...
public class Netzwerkverbindung implements EventListener {
    ...
    HashMap<Integer, List<String[]>> zeitstempel;
    ...
    private void ClientPreStart() {
        ...
        registerMethod(3, new Executable() {
            ...
            public void run(Datapackage msg, Socket socket) {
                ...
                int intEingabe = CLI.getCarType();
                if (intEingabe < 1 || intEingabe > autoListe.size()) {
        ...
        registerMethod(7, new Executable() {
            ...
            public void run(Datapackage msg, Socket socket) {
                ...
                //TODO: dynamische Kartengroeße
                //Bekomme die Spaltenanzahl
                for (int k = 0; k < 4; k++) {
                    hilfs[k] = daten[intHilf + k];
                }
                intHilf += 4;
                optionen.maxBildschirmSpalten = convertByteArrayToInt(hilfs);

                //Bekomme die Zeilenanzahl
                for (int k = 0; k < 4; k++) {
                    hilfs[k] = daten[intHilf + k];
                }
                intHilf += 4;
                optionen.maxBildschirmZeilen = convertByteArrayToInt(hilfs);
        ...
        registerMethod(8, new Executable() {
            ...
            public void run(Datapackage msg, Socket socket) {
                ...
                if (optionen.addClientsInformation == null) {
                    optionen.addClientsInformation = new HashMap<Integer, Object[]>();
                }
                Object[] objectArray = new Object[4];
                objectArray[0] = AutoTyp;
                objectArray[1] = dworldX;
                objectArray[2] = dworldY;
                objectArray[3] = direction;
                optionen.addClientsInformation.put(SpielerID, objectArray);
...
//				});
    ...
    private void ServerPreStart() {
        ...
        registerMethod(LOGIN_ID, new Executable() {
            ...
            public void run(Datapackage msg, Socket socket) {
                ...
                if (id == 0) {
                   ...
                   /* try {
                        Thread.sleep(8000); //Wenn OpenGL auf dem Client hochgefahren ist
                    } catch (InterruptedException e) {
                        // TODO Auto-generated catch block
                        e.printStackTrace();
                    }*/
        ...
        registerMethod(6, new Executable() {
            ...
            public void run(Datapackage msg, Socket socket) {
                ...
                //System.out.println(up + " " + down + " " + left + " " + right + " " + spielerID);

                //StarteEvent
                event.notify("key_event", spielerID, up, down, left, right);

                //Jetzt timestamp setzen
                String direction = convertDirection(up, down, left, right);
                zeitstempelSetzen(spielerID, direction);
            }
        });
    ...
    /**
     * Konvertiert die gedrueckten Tasten in die korrespondierenden Strings
     *
     * @param up
     * @param down
     * @param left
     ...
     * @return
     */
    String convertDirection(boolean up, boolean down, boolean left, boolean right) {
        String richtung;
        if (down && left) {
            richtung = "down+left";
        } else if (up && right) {
...
//		      }
    ...
    protected void ServerSendeSpielfeld(Socket socket) {
        ...
        byte[] kartenbyte = new byte[optionen.maxBildschirmZeilen * 4 * optionen.maxBildschirmSpalten + 8]; // +8 = 4 für Anzahl Spalten und 4 für Anzahl Zeilen

        ByteBuffer buff = ByteBuffer.wrap(kartenbyte);

        //Kartengroeße konvertieren 1.Stelle Spaltenanzahl 2.Stelle Zeilenanzahl
        buff.put(convertIntToByteArray(optionen.maxBildschirmSpalten));
        buff.put(convertIntToByteArray(optionen.maxBildschirmZeilen));
        length += 8;
...
//		}
        ...
        if (eventType == "aendere_status_beendet") {//fehtl noch auffangFunktion
            ...
            //Zeitstempel der Spieler speichern
    ...
    /**
     * Initialisiert die Datenstruktur des Zeitstempels
     */
    void initZeitstempel() {
        zeitstempel = new HashMap<>();
    }

    JSONObject convertZeitstempel(int player) {
        JSONObject obj = new JSONObject();
        List<JSONObject> list = new ArrayList<>();

        for (var entry : zeitstempel.get(player)) {
            JSONObject subObj = new JSONObject();
            subObj.put("timestamp", entry[1]);
            subObj.put("direction", entry[0]);
            list.add(subObj);
        }

        obj.put("timestamps", list);

        return obj;
    }

    /**
     * Speichert den Zeitstempel in einer JSON-Datei ab
     */
    void zeitstempelSpeichern() {
        String saveFile = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new java.util.Date()) + ".json";

        for (int player = 1; player <= zeitstempel.size(); player++) {
            //Map to JSONObject
            String playerFile = "player_" + player + "_" + saveFile;

            List<String[]> entry = zeitstempel.get(player);
            String letzteRichtung = entry.get(entry.size() - 1)[0];
            entry.add(new String[]{letzteRichtung, "" + Long.MAX_VALUE});
            zeitstempel.put(player, entry);

            JSONObject obj = convertZeitstempel(player);
    ...
    /**
     * Erzeugt einen neuen Zeitstempel für den angegebenen Spieler mit angegebener Richtung
     *
     * @param spielerID Spieler, für den der Zeitstempel gesetzt werden soll
     * @param direction Richtung, in die der Spieler faehrt
     */
    void zeitstempelSetzen(int spielerID, String direction) {
        if (zeitstempel == null || zeitstempel.size() == 0) {
            initZeitstempel();
        }
        //SpielerID existiert noch nicht -> Eintrag erstellen mit neuer Liste
        if (!zeitstempel.containsKey(spielerID)) {
            zeitstempel.put(spielerID, new ArrayList<>());
            zeitstempel.get(spielerID).add(new String[]{direction,
                    "" + System.nanoTime()});
        } else {
            List<String[]> current = zeitstempel.get(spielerID);
            //Nur neuen Stempel erzeugen, wenn neue Richtung geschickt wurde
            if (!current.get(current.size() - 1)[0].equals(direction)) {
                System.out.println("Zeitstempel " + spielerID);
                zeitstempel.get(spielerID).add(new String[]{direction,
                        "" + System.nanoTime()});
            }
        }
    ...
    void merkeSpielStart() {
        String start = "" + System.nanoTime();
        for (int player = 1; player <= optionen.maxAnzahlSpieler; player++) {
            List<String[]> entry = new ArrayList<>();
            entry.add(new String[]{"null", start});
            zeitstempel.put(player, entry);
        }
    }
